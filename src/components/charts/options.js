...
export function monthlyIrradiance(s) {
  ...
  return {
    ...
    series: [
      barS("GHI", m.ghi, C.ghi),
      barS("DNI", m.dni, C.dni),
      barS("DHI", m.dhi, C.dhi),
    ],
  };
}
...
export function sunPath(s) {
  ...
  return {
    ...
    tooltip: {
      trigger: "item",
      formatter: (p) => `az ${p.value[0]}° · el ${p.value[1]}°`,
    },
    xAxis: {
      type: "value",
      name: "Azimuth °",
...
export function irradianceHeatmap(s) {
  const data = [];
  let max = 0;
  s.heatmap.forEach((row, mi) =>
    row.forEach((v, h) => {
      data.push([h, mi, Math.round(v)]);
      if (v > max) max = v;
    })
  );
  return {
    ...
    tooltip: {
      position: "top",
      formatter: (p) =>
        `${MONTHS[p.value[1]]} ${p.value[0]}:00 — ${p.value[2]} W/m²`,
    },
...
export function tiltYield(s) {
  ...
  return {
    ...
    tooltip: {
      trigger: "axis",
      formatter: (p) => `${p[0].axisValue}° → ${p[0].value} kWh/m²/yr`,
    },
    ...
    series: [
      {
        ...
        smooth: true,
        showSymbol: false,
        lineStyle: { color: C.ghi, width: 2 },
        areaStyle: { color: "rgba(230,161,58,0.12)" },
        markPoint: {
          symbolSize: 46,
          data: [
            {
              coord: [String(s.optimal_tilt), t.poa[oi]],
              value: `${s.optimal_tilt}°`,
              itemStyle: { color: C.dni },
            },
          ],
        },
      },
...
export function windRose(w) {
  ...
  return {
    ...
    series: r.speed_bins.map((sb, i) => ({
      ...
      stack: "w",
      data: r.matrix.map((row) => row[i]),
      itemStyle: { color: ROSE[i] },
    })),
  };
}
...
export function weibullHist(w) {
  ...
  return {
    ...
    series: [
      barS("Observed", density, C.wind),
      lineS(`Weibull k=${k}`, pdf, C.dni),
    ],
  };
}
    ...
    series: [
      {
        ...
        axisLabel: { color: DIM, distance: 14 },
        axisTick: { show: false },
        splitLine: { lineStyle: { color: DIM } },
        pointer: { show: false },
        anchor: { show: false },
...
export function tempBand(cl) {
  ...
  return {
    ...base(),
    tooltip: { trigger: "axis" },
    legend: { data: ["Mean"], textStyle: { color: TXT } },
    ...
    series: [
      ...
      },
      {
        ...
        stack: "band",
        lineStyle: { opacity: 0 },
        areaStyle: { color: "rgba(108,157,255,0.18)" },
        symbol: "none",
        silent: true,
      },
...
export function degreeDays(cl) {
  ...
  return {
    ...
    series: [
      barS("Heating (HDD)", m.hdd, C.hdd),
      barS("Cooling (CDD)", m.cdd, C.cdd),
    ],
  };
}
...
export function durationCurve(cl) {
  ...
  const pts = cl.duration_curve.map((t, i) => [
    Math.round((i / (n - 1)) * 1000) / 10,
    t,
  ]);
  return {
    ...base(),
    ...
    tooltip: {
      trigger: "axis",
      formatter: (p) =>
        `${p[0].value[1]} °C exceeded ${p[0].value[0]}% of hours`,
    },
    xAxis: {
      type: "value",
      name: "% of hours",
...
export function precipBars(rain) {
  return {
    ...base(),
    legend: { show: false },
    tooltip: {
      trigger: "axis",
      formatter: (p) => `${p[0].axisValue}: ${p[0].value} mm`,
    },
    xAxis: { type: "category", data: MONTHS, ...axis() },
    yAxis: { type: "value", name: "mm / month", ...axis() },
    series: [barS("Precipitation", rain.monthly_mm, "#3f9bd6")],
  };
}
...
export function pollutants(aq) {
  ...
  return {
    ...
    series: [
      lineS("PM2.5", m.pm2_5, C.pm),
      lineS("PM10", m.pm10, C.pm10),
      lineS("NO₂", m.nitrogen_dioxide, C.no2),
      lineS("O₃", m.ozone, C.o3),
    ],
  };
}
...
export function daysOverLimit(aq) {
  ...
  return {
    ...
    series: [
      barS("Days", [d.pm2_5, d.pm10, d.nitrogen_dioxide, d.ozone], C.pm),
    ],
  };
}
