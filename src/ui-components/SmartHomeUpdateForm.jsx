...
import { getSmartHome } from "../graphql/queries";
import { updateSmartHome } from "../graphql/mutations";
...
export default function SmartHomeUpdateForm(props) {
  const {
    id: idProp,
    smartHome: smartHomeModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = smartHomeRecord
      ? { ...initialValues, ...smartHomeRecord }
      : initialValues;
    setOwners(cleanValues.owners ?? []);
    setCurrentOwnersValue("");
    setCountry(cleanValues.country);
    setZip(cleanValues.zip);
    setStreetCode(cleanValues.streetCode);
    setHouseNumber(cleanValues.houseNumber);
    setSuffix(cleanValues.suffix);
    setAddress(cleanValues.address);
    setDescription(cleanValues.description);
    setOwnerName(cleanValues.ownerName);
    setErrors({});
  };
  const [smartHomeRecord, setSmartHomeRecord] =
    React.useState(smartHomeModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await client.graphql({
              query: getSmartHome.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getSmartHome
        : smartHomeModelProp;
      setSmartHomeRecord(record);
    };
    queryData();
  }, [idProp, smartHomeModelProp]);
  React.useEffect(resetStateValues, [smartHomeRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await client.graphql({
            query: updateSmartHome.replaceAll("__typename", ""),
            variables: {
              input: {
                id: smartHomeRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "SmartHomeUpdateForm")}
      {...rest}
    >
      <ArrayField
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || smartHomeModelProp)}
